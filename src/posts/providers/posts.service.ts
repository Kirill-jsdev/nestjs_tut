...
export class PostsService {
  ...
  public async update(postId: number, patchPostDto: PatchPostDto) {
    ...
    try {
      tags = patchPostDto.tags ? await this.tagsService.findMultipleTags(patchPostDto.tags) : undefined;
    } catch (error) {
    ...
    if ((patchPostDto.tags && tags === null) || (tags && tags.length !== (patchPostDto.tags?.length ?? 0))) {
